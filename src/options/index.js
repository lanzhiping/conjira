...
const showSecrets = async ({ list, search }) => {
  loadSecretBySearch(search.value).then((secrets) => renderList(list, secrets));
};

const renderList = (listElement, secrets) => {
  const secretElements = secrets
    .map(({ _id, name, account }) => {
      return `
...
const onAddNewClick = async (event, target, elements) => {
  ...
  const inputElements = [...target.children].filter(
    (el) => el.localName === "input"
  );
  const newSecret = inputElements.reduce(
    (secret, input) => ({
      ...secret,
